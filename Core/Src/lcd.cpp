...
LCD_CtrlLinesConfig(void) {
	...
	*/
	...
	/* GPIOE configuration */
	GPIO_InitStructure.Pin = GPIO_PIN_2 | GPIO_PIN_7  | GPIO_PIN_8  | GPIO_PIN_9  | GPIO_PIN_10 |
...
LCD_FSMCConfig(void) {
	...
	FSMC_NORSRAM_Init(&hfsmc, &FSMC_NORSRAMInitStructure); 

	FSMC_NORSRAMTimingInitStructure.AddressSetupTime = 15;//1;
	FSMC_NORSRAMTimingInitStructure.AddressHoldTime = 1;	   
	FSMC_NORSRAMTimingInitStructure.DataSetupTime = 15;//1;
	FSMC_NORSRAMTimingInitStructure.BusTurnAroundDuration = 0;
	FSMC_NORSRAMTimingInitStructure.CLKDivision = 2;
	FSMC_NORSRAMTimingInitStructure.DataLatency = 2;
	FSMC_NORSRAMTimingInitStructure.AccessMode = FSMC_ACCESS_MODE_A;
	FSMC_NORSRAM_Timing_Init(&hfsmc, &FSMC_NORSRAMTimingInitStructure, FSMC_NORSRAM_BANK1);

	/* Enable FSMC Bank4_SRAM Bank */
	__FSMC_NORSRAM_ENABLE(&hfsmc, FSMC_NORSRAM_BANK1);  
}
...
*******************************************************************************/
void 
PutChar(uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
...
*******************************************************************************/
void 
GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor) {
    uint8_t TempChar;
    do
    {
        TempChar = *str++;  
...
*******************************************************************************/
__inline void 
LCD_WriteReg(uint16_t LCD_Reg,uint16_t LCD_RegValue) { 
